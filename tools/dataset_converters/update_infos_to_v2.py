...
def generate_nuscenes_driving_camera_instances(info, nusc):
...
def update_nuscenes_driving_infos(pkl_path, out_dir):
    camera_types = [
        'back_left_camera',
        'left_camera',
        'front_left_camera',
    ...
    for i, ori_info_dict in enumerate(
        ...
        if 'gt_boxes' in ori_info_dict:
            ...
            for i in range(num_instances):
                ...
                temp_data_info['instances'].append(empty_instance)
            temp_data_info[
                'cam_instances'] = generate_nuscenes_driving_camera_instances(
                    ori_info_dict, nusc)
        if 'pts_semantic_mask_path' in ori_info_dict:
            temp_data_info['pts_semantic_mask_path'] = Path(
                ori_info_dict['pts_semantic_mask_path']).name
...
def update_pkl_infos(dataset, out_dir, pkl_path):
    ...
    elif dataset.lower() == 'nuscenes-driving':
        update_nuscenes_driving_infos(pkl_path=pkl_path, out_dir=out_dir)
