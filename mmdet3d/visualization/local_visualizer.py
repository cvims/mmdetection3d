...
#             line_set = geometry.LineSet.create_from_oriented_bounding_box(
#                 box3d)
#             color = np.array(bbox_color) / 255.
#             if isinstance(line_set, o3d.cuda.pybind.geometry.LineSet):
#                 line_set.colors = o3d.utility.Vector3dVector(np.array(color))
#             else:
...
# @VISUALIZERS.register_module()
# class Det3DLocalVisualizer(DetLocalVisualizer):
#     """Jupyter-ready MMDetection3D visualizer using Open3D 0.19 draw()."""
...
#                    frame_cfg: dict = dict(size=1, origin=[0, 0, 0]),
#                    mode: str = 'xyz') -> None:
#         """Set or replace the point cloud for visualization."""
#         check_type('points', points, np.ndarray)
#         if pcd_mode != Coord3DMode.DEPTH:
#             points = Coord3DMode.convert(points, pcd_mode, Coord3DMode.DEPTH)
...
#             pcd.points = o3d.utility.Vector3dVector(points[:, :3])
#             colors = points[:, 3:6]
#             if not ((colors >= 0.0) & (colors <= 1.0)).all():
#                 colors /= 255.0
#         else:
#             raise NotImplementedError(f'Unknown mode: {mode}')
#         pcd.colors = o3d.utility.Vector3dVector(colors)
#         self.pcd = pcd
#         self.points_colors = colors

#         # add coordinate frame
#         frame = geometry.TriangleMesh.create_coordinate_frame(**frame_cfg)
#         self.geometries = [frame, self.pcd]

#     @master_only
#     def draw_bboxes_3d(self,
#                        bboxes_3d: BaseInstance3DBoxes,
#                        bbox_color: Union[Tuple[float], List[Tuple[float]]] = (0, 1, 0),
#                        rot_axis: int = 2,
#                        center_mode: str = 'lidar_bottom',
#                        mode: str = 'xyz') -> None:
...
#         if isinstance(bbox_color, tuple):
#             bbox_color = [bbox_color] * len(bboxes_3d)
...
#             yaw = np.zeros(3)
#             yaw[rot_axis] = box[6]
#             rot = geometry.get_rotation_matrix_from_xyz(yaw)
#             if center_mode == 'lidar_bottom':
#                 center[rot_axis] += dims[rot_axis] / 2
#             elif center_mode == 'camera_bottom':
#                 center[rot_axis] -= dims[rot_axis] / 2
#             obb = geometry.OrientedBoundingBox(center, rot, dims)
#             line_set = geometry.LineSet.create_from_oriented_bounding_box(obb)
#             color = np.array(bbox_color[i]) / 255. if np.max(bbox_color[i]) > 1 else np.array(bbox_color[i])
#             line_set.paint_uniform_color(color)
#             self.geometries.append(line_set)

#             # recolor points inside box
#             if hasattr(self, 'pcd') and mode == 'xyz':
#                 indices = obb.get_point_indices_within_bounding_box(self.pcd.points)
#                 for idx in indices:
#                     self.points_colors[idx] = color
#                 self.pcd.colors = o3d.utility.Vector3dVector(self.points_colors)

#     @master_only
#     def draw_seg_mask(self, seg_points: np.ndarray) -> None:
#         """Add per-point segmentation mask."""
#         seg_points_copy = copy.deepcopy(seg_points)
#         if hasattr(self, 'pcd'):
#             offset = (np.array(self.pcd.points).max(0) -
#                       np.array(self.pcd.points).min(0))[0] * 1.2
#             seg_points_copy[:, 0] += offset
#         self.set_points(seg_points_copy, pcd_mode=2, mode='xyzrgb')
#         self.geometries.append(self.pcd)

#     @master_only
#     def show(self, save_path: Optional[str] = None, *args, **kwargs) -> None:
#         """Render all geometries in Jupyter."""
#         if not self.geometries:
#             print("Nothing to draw!")
#             return
#         # o3d.visualization.draw(self.geometries)

#         # Initialize the Open3D GUI application
#         o3d.visualization.gui.Application.instance.initialize()
#         vis = o3d.visualization.O3DVisualizer("PointCloud", 800, 600)
#         vis.show(True)       
#         for i, geo in enumerate(self.geometries):
#             vis.add_geometry(str(i), geo)
...
#         if vis_task == 'lidar_seg' and ignore_index is not None and 'pts_semantic_mask' in data_sample.gt_pts_seg:
#             keep_index = data_sample.gt_pts_seg.pts_semantic_mask != ignore_index
#         else:
#             keep_index = None

#         gt_data_3d = pred_data_3d = gt_img_data = pred_img_data = None
...
#                     data_sample.metainfo, vis_task, show_pcd_rgb, palette
#                 )
#             if 'gt_instances' in data_sample and len(data_sample.gt_instances) > 0:
#                 assert 'img' in data_input
#                 img = data_input['img']
#                 if isinstance(img, Tensor):
#                     img = img.permute(1, 2, 0).numpy()[..., [2, 1, 0]]
#                 gt_img_data = self._draw_instances(img, data_sample.gt_instances, classes, palette)
#             if 'gt_pts_seg' in data_sample and vis_task == 'lidar_seg':
#                 assert classes is not None, 'Class information is required for semantic segmentation.'
#                 assert 'points' in data_input
#                 self._draw_pts_sem_seg(data_input['points'], data_sample.gt_pts_seg, palette, keep_index)
...
#                     vis_task, show_pcd_rgb, palette
#                 )
#             if 'pred_instances' in data_sample and 'img' in data_input and len(data_sample.pred_instances) > 0:
#                 pred_instances = data_sample.pred_instances
#                 pred_instances = pred_instances[pred_instances.scores > pred_score_thr].cpu()
#                 img = data_input['img']
#                 if isinstance(img, Tensor):
#                     img = img.permute(1, 2, 0).numpy()[..., [2, 1, 0]]
#                 pred_img_data = self._draw_instances(img, pred_instances, classes, palette)
#             if 'pred_pts_seg' in data_sample and vis_task == 'lidar_seg':
#                 assert classes is not None, 'Class information is required for semantic segmentation.'
#                 assert 'points' in data_input
#                 self._draw_pts_sem_seg(data_input['points'], data_sample.pred_pts_seg, palette, keep_index)
